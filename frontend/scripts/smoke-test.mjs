const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4960';
...
const adminCookie = await login('admin@maritime-port-operations.local', 'admin123');
const managerCookie = await login('manager@maritime-port-operations.local', 'manager123');
const analystCookie = await login('analyst@maritime-port-operations.local', 'analyst123');
